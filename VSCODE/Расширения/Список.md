...
- `Color Highlight (Sergii N)` - подсветка цветов
...
- `Image preview (Kiss Tamás)` - предпросмотр картинок
...
- `Todo Tree (Gruntfuggly)` - подскажет где оставлены TODO
- `Vscode Google Translate (funkyremi)` - переводчик
- `vscode-pdf (tomoki1207)` - открывает файлы pdf
...
- `Tailwind CSS IntelliSense (Tailwind Labs)` - подсказки для Tailwind
